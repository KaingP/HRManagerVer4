...
function getOverlappingStandardSlots(
    startStr: string,
    endStr: string,
): string[] {
...
export class ShiftScheduler {
    ...
    public optimize(): any {
        ...
        const active_types = this.config.active_types || ["Phong", "Ngoai"];
        const phong_shifts = active_types.includes("Phong")
            ? this.shifts.filter((s) => s.type === "Phong")
            : [];
        for (const s of phong_shifts) {
            if (s.active === false) continue;
            const s_copy = { ...s };
            if (this.config.start_date) {
                const computedDate = calculateDateForDay(
            ...
            const chinh_req =
                s.chinh_count !== undefined
                    ? s.chinh_count
                    : this.config.phong_chinh_count !== undefined
                      ? this.config.phong_chinh_count
                      : s.required_count || 4;
            const dp_req =
                s.dp_count !== undefined
                    ? s.dp_count
                    : this.config.phong_dp_count !== undefined
                      ? this.config.phong_dp_count
                      : 1;
            ...
            s_copy.overlapping_slots = Array.from(
                new Set(
                    [
                        ...(s.overlapping_slots || []),
                        ...standardSlots,
        ...
        if (this.config.enable_ca_ngoai && active_types.includes("Ngoai")) {
            const custom_ngoai = this.config.custom_ca_ngoai;
            if (custom_ngoai && Array.isArray(custom_ngoai)) {
                custom_ngoai.forEach((c, idx) => {
                    const c_id =
                        c.id || `NGOAI_${String(idx + 1).padStart(2, "0")}`;
                    const c_day = c.day || "Thứ 7";
                    const c_date = this.config.start_date
                        ? calculateDateForDay(this.config.start_date, c_day)
                        : c.date || "Tuần F&B";
                    ...
                    const overlap_slots = getOverlappingStandardSlots(
                        c_start,
                        c_end,
                    );
                ...
                const ngoai_shifts = this.shifts.filter(
                    (s) => s.type === "Ngoai",
                );
                for (const s of ngoai_shifts) {
                    ...
                    s_copy.chinh_count = Math.max(
                        1,
                        (s.required_count || 3) - 1,
                    );
                    s_copy.dp_count = 1;
                    s_copy.required_count =
                        (s_copy.chinh_count || 0) + (s_copy.dp_count || 0);
        ...
        const total_demand = active_shifts.reduce(
            (sum, s) => sum + (s.required_count || 0),
            0,
        );
        ...
        const getEligibleMembers = (
            ...
            return this.members.filter((m) => {
                ...
                const user_max =
                    this.config.max_shifts_per_member !== undefined
                ...
                const current_day_count =
                    (member_days[m.member_id] &&
                        member_days[m.member_id][day]) ||
                    0;
                if (
                    current_day_count >=
                    max_per_day + (allowExtraShift ? 1 : 0)
                )
                    return false;

                // Double booking check across overlapping shifts on same day
                const doubleBooked = active_shifts.some(
                    (otherShift, otherIdx) => {
                        if (otherIdx === shiftIdx) return false;
                        if (otherShift.day !== day) return false;
                        const other_overlap = otherShift.overlapping_slots || [
                            otherShift.slot,
                        ];
                        const hasOverlap = overlap_slots.some((sl) =>
                ...
                // Availability check
                const is_reg_free = overlap_slots.some(
        ...
        const scoreCandidate = (
            ...
            const is_committed_any = overlap_slots.some(
                (sl) => m.committed_slots[`${day}|${sl}`] === true,
            );
            if (is_committed_any) {
            ...
            const has_vehicle =
                m.vehicle.toLowerCase().includes("xe máy") ||
                m.vehicle.toLowerCase().includes("tự đi");
            if (
                has_vehicle &&
                (overlap_slots.includes("7h - 9h") || shift.type === "Ngoai")
            ) {
        ...
        for (let trial = 0; trial < numTrials; trial++) {
            ...
            const member_days: {
                [memberId: string]: { [day: string]: number };
            } = {};
            ...
            for (const idx of shiftIndices) {
                ...
                while (current_assignments[idx].length < req) {
                    let candidates = getEligibleMembers(
                        idx,
                        current_assignments,
                        member_totals,
                        member_days,
                    ...
                    member_days[chosen.member_id][shift.day] =
                        (member_days[chosen.member_id][shift.day] || 0) + 1;
                }
            }
            ...
            for (let idx = 0; idx < active_shifts.length; idx++) {
                ...
                while (current_assignments[idx].length < req) {
                    let swapped = false;
                    const overlap_slots = shift.overlapping_slots || [
                        shift.slot,
                    ];
                    const neededEligible = this.members.filter((m) => {
                        return overlap_slots.some(
                            (sl) =>
                                m.availability[`${shift.day}|${sl}`] === true,
                        );
                    });

                    for (const cand of neededEligible) {
                        if (current_assignments[idx].includes(cand.member_id))
                            continue;

                        for (
                            let otherIdx = 0;
                            otherIdx < active_shifts.length;
                            otherIdx++
                        ) {
                            if (otherIdx === idx) continue;
                            if (
                                current_assignments[otherIdx].includes(
                                    cand.member_id,
                                )
                            ) {
                                const replacements = getEligibleMembers(
                                    otherIdx,
                                    current_assignments,
                                    member_totals,
                                    member_days,
                                    ...
                                    (rep) => rep.member_id !== cand.member_id,
                                );

                                if (replacements.length > 0) {
                                    const rep = replacements[0];
                                    // Remove cand from otherIdx and add rep
                                    current_assignments[otherIdx] =
                                        current_assignments[otherIdx].filter(
                                            (id) => id !== cand.member_id,
                                        );
                                    member_days[cand.member_id][
                                        active_shifts[otherIdx].day
                                    ] = Math.max(
                                        0,
                                        (member_days[cand.member_id][
                                            active_shifts[otherIdx].day
                                    ...
                                    current_assignments[otherIdx].push(
                                        rep.member_id,
                                    );
                                    member_totals[rep.member_id]++;
                                    member_days[rep.member_id][
                                        active_shifts[otherIdx].day
                                    ] =
                                        (member_days[rep.member_id][
                                            active_shifts[otherIdx].day
                                    ...
                                    // Place cand in idx
                                    current_assignments[idx].push(
                                        cand.member_id,
                                    );
                                    member_days[cand.member_id][shift.day] =
                                        (member_days[cand.member_id][
                                            shift.day
                                        ] || 0) + 1;
                                    swapped = true;
                    ...
                    if (!swapped) {
                        ...
                        const user_max_limit =
                            this.config.max_shifts_per_member || 5;
                        const standbyEligible = this.members.filter((m) => {
                            if (!m.is_standby) return false;
                            if (current_assignments[idx].includes(m.member_id))
                                return false;
                            const cur_count = member_totals[m.member_id] || 0;
                            if (cur_count >= user_max_limit) return false;
                            const dCount =
                                (member_days[m.member_id] &&
                                    member_days[m.member_id][shift.day]) ||
                                0;
                            return dCount < max_per_day + 1;
                        });

                        if (standbyEligible.length > 0) {
                            standbyEligible.sort(
                                (a, b) =>
                                    (member_totals[a.member_id] || 0) -
                                    (member_totals[b.member_id] || 0),
                            );
            ...
            this.members.forEach((m) => {
                ...
                active_shifts.forEach((shift, j) => {
                    if (current_assignments[j].includes(m.member_id)) {
                        ...
                        const is_commit = overlap_slots.some(
                            (sl) =>
                                m.committed_slots[`${shift.day}|${sl}`] ===
                                true,
                        );
                        if (is_commit) {
                            trial_score +=
                                (this.config.weight_committed || 15) * 4;
                        }
                        const has_vehicle =
                            m.vehicle.toLowerCase().includes("xe máy") ||
                            m.vehicle.toLowerCase().includes("tự đi");
                        if (
                            has_vehicle &&
                            (overlap_slots.includes("7h - 9h") ||
                                shift.type === "Ngoai")
                        ) {
                            trial_score +=
                                (this.config.weight_transport || 3) * 2;
                        }
                    }
                });
            });
        ...
        active_shifts.forEach((shift, j) => {
            ...
            const raw_assigned = this.members.filter((m) =>
                raw_assigned_ids.includes(m.member_id),
            );
            ...
            raw_assigned.sort((a, b) => {
                const a_commit = overlap_slots.some(
                    (sl) => a.committed_slots[`${shift.day}|${sl}`] === true,
                );
                const b_commit = overlap_slots.some(
                    (sl) => b.committed_slots[`${shift.day}|${sl}`] === true,
                );
            ...
            if (assigned_members.length > 0) {
                const hr_members = assigned_members.filter(
                    (m) =>
                        (m.department.includes("Nhân sự") ||
                            m.department.includes("Sự kiện")) &&
                        m.role === "Chính",
                );
                if (hr_members.length > 0) {
                    shift_leader = hr_members[0].name;
                } else {
                    const chinh_members = assigned_members.filter(
                        (m) => m.role === "Chính",
                    );
                    shift_leader =
                        chinh_members.length > 0
                            ? chinh_members[0].name
                            : assigned_members[0].name;
                }
            }
            ...
            assigned_shifts.push({
                ...
                chinh_assigned_count: assigned_members.filter(
                    (m) => m.role === "Chính",
                ).length,
                dp_assigned_count: assigned_members.filter(
                    (m) => m.role === "Dự phòng",
                ).length,
                shift_leader,
                is_filled:
                    assigned_members.length >= (shift.required_count || 0),
            });
        });
        ...
        const member_stats = this.members.map((member) => {
            ...
            const shifts_assigned = assigned_shifts.filter((s) =>
                assigned_ids.includes(s.shift_id),
            );

            const phong_count = shifts_assigned.filter(
                (s) => s.type === "Phong",
            ).length;
            const ngoai_count = shifts_assigned.filter(
                (s) => s.type === "Ngoai",
            ).length;
            const commit_matched = shifts_assigned.filter((s) => {
                const overlap = s.overlapping_slots || [s.slot];
                return overlap.some(
                    (sl) => member.committed_slots[`${s.day}|${sl}`] === true,
                );
            }).length;
        ...
        return {
            ...
            summary: {
                ...
                total_assignments: assigned_shifts.reduce(
                    (sum, s) => sum + s.assigned_count,
                    0,
                ),
                total_members: num_members,
                avg_shifts_per_member: parseFloat(
                    (
                        member_stats.reduce(
                            (sum, m) => sum + m.total_shifts,
                            0,
                        ) / num_members
                    ).toFixed(2),
                ),
                phong_shifts_filled: assigned_shifts.filter(
                    (s) => s.type === "Phong" && s.is_filled,
                ).length,
                total_phong_shifts: assigned_shifts.filter(
                    (s) => s.type === "Phong",
                ).length,
                ngoai_shifts_filled: assigned_shifts.filter(
                    (s) => s.type === "Ngoai" && s.is_filled,
                ).length,
                total_ngoai_shifts: assigned_shifts.filter(
                    (s) => s.type === "Ngoai",
                ).length,
            },
        };
    ...
    private runAudit(assigned_shifts: any[], member_stats: any[]): any {
        ...
        for (let s1_idx = 0; s1_idx < assigned_shifts.length; s1_idx++) {
            ...
            for (
                ...
                if (s1.day === s2.day) {
                    ...
                    const intersection = slots1.filter((sl: string) =>
                        slots2.includes(sl),
                    );

                    if (intersection.length > 0) {
                        const m_set1 = s1.assigned_members.map(
                            (m: any) => m.member_id,
                        );
                        const m_set2 = s2.assigned_members.map(
                            (m: any) => m.member_id,
                        );
                        const overlap_m = m_set1.filter((mId: string) =>
                            m_set2.includes(mId),
                        );
        ...
        assigned_shifts.forEach((s) => {
            ...
            s.assigned_members.forEach((m: any) => {
                ...
                if (orig) {
                    const is_free_all = overlap_slots.every(
                        (sl: string) =>
                            orig.availability[`${s.day}|${sl}`] === true,
                    );
                    if (!is_free_all) {
        ...
        assigned_shifts.forEach((s) => {
            s.assigned_members.forEach((m: any) => {
                ...
                member_day_map[m.member_id][s.day] =
                    (member_day_map[m.member_id][s.day] || 0) + 1;
            });
        });
        ...
        const mean_c =
            shift_counts.length > 0
                ? shift_counts.reduce((a, b) => a + b, 0) / shift_counts.length
                : 0;
        const variance =
            shift_counts.length > 0
                ? shift_counts.reduce(
                      (sum, c) => sum + Math.pow(c - mean_c, 2),
                      0,
                  ) / shift_counts.length
                : 0;
        ...
        const is_passed =
            conflicts.length === 0 &&
            availability_violations.length === 0 &&
            empty_rooms.length === 0;
        ...
        return {
            ...
            fairness_metrics: {
                ...
                fairness_score: Math.max(
                    0,
                    parseFloat((100 - std_dev * 15).toFixed(1)),
                ),
            },
        };
    }
    ...
    private buildContingency(assigned_shifts: any[]): any[] {
        ...
        assigned_shifts.forEach((s) => {
            const assigned_m_ids = s.assigned_members.map(
                (m: any) => m.member_id,
            );
            const overlap_slots = s.overlapping_slots || [s.slot];
            const backups: any[] = [];
            ...
            this.members.forEach((m) => {
                ...
                const is_free_all = overlap_slots.every(
                    (sl) => m.availability[`${s.day}|${sl}`] === true,
                );
                if (is_free_all) {
                    backups.push({
                        member_id: m.member_id,
            ...
            contingency.push({
                ...
                slot: s.slot,
                location: s.location,
                current_assigned: s.assigned_members.map(
                    (m: any) => `${m.name} (${m.role})`,
                ),
